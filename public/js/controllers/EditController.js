...
appControllers.controller('EditController',function(TagService,$scope,BookmarkService,Helpers,$stateParams,$state,$modal,CONSTANT,$http,focus,toaster){
    ...
    $scope.updateBookmark=function(bookmark){
      ...
      console.log("Bookmark "+JSON.stringify(bookmark));
       if(Helpers.undefined_or_empty(bookmark.link)){$scope.editBookmarkMessage='Nay! looks like you forgot to enter Medicine'; return;}
       if(Helpers.undefined_or_empty(bookmark.description)){$scope.editBookmarkMessage='Please fill in Indications'; return;}
       if(bookmark.inputTags.length < 1){$scope.editBookmarkMessage='Nay! we need at least one Patient'; return;}
       ...
       .then(function(response){
            ...
            function(error){ console.log("Error while updating Record"); }
          );
    }
